...
func (l *LocalQrouterServer) Finish2PhaseCommit(ctx context.Context, request *protos.TwoPCRequest) (*emptypb.Empty, error) {
	...
	dbPool := pool.NewDBPool(shardMapping, startupParams, "") // тут используется функция алокатора
	// todo mattthey fix it (maybe need user + db for use method pkg/rulemgr/rulemgr.go:18 RulesMgr.MatchKeyBackend)
	dbPool.SetRule(routerConfig.BackendRules[0])
